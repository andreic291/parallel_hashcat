...
# Script to test parallel container 2 performance (second half)

HASH_FILE="test.hc22000"
WORDLIST_FILE="wordlists/wordlist_part2.txt"
RESULT_FILE="results/parallel_part2_result.txt"

echo "=== PARALLEL CONTAINER 2 PERFORMANCE TEST ===" | tee $RESULT_FILE
echo "Testing second half of wordlist" | tee -a $RESULT_FILE
...
echo "Running hashcat on wordlist part 2..." | tee -a $RESULT_FILE
...
if [ -n "$CRACKED_LINE" ]; then
    echo "STATUS: PASSWORD FOUND IN PART 2" | tee -a $RESULT_FILE
    ...
    # Signal that password was found
    touch results/password_found_part2
else
    echo "STATUS: PASSWORD NOT FOUND IN PART 2" | tee -a $RESULT_FILE
fi 