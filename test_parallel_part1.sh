...
# Script to test parallel container 1 performance (first half)

HASH_FILE="test.hc22000"
WORDLIST_FILE="wordlists/wordlist_part1.txt"
RESULT_FILE="results/parallel_part1_result.txt"

echo "=== PARALLEL CONTAINER 1 PERFORMANCE TEST ===" | tee $RESULT_FILE
echo "Testing first half of wordlist" | tee -a $RESULT_FILE
...
echo "Running hashcat on wordlist part 1..." | tee -a $RESULT_FILE
...
if [ -n "$CRACKED_LINE" ]; then
    echo "STATUS: PASSWORD FOUND IN PART 1" | tee -a $RESULT_FILE
    ...
    touch results/password_found_part1
else
    echo "STATUS: PASSWORD NOT FOUND IN PART 1" | tee -a $RESULT_FILE
fi 